...
tm_shape(maus_filtered%>%filter(geonunit == "Argentina"))+
  tm_polygons()+
  tm_shape(st_buffer(maus_filtered%>%filter(geonunit == "Argentina"), 2000))+
  tm_borders()+
...
Last, let's count how many mines are in each adm1, so that we can callibrate the proportion of how many samples to take in non-mining areas.
```{r}
adm1s_filtered$n_mines = lengths(st_intersects(adm1s_filtered, rbind(maus_filtered%>%select(geom), snp_filtered%>%select(geom))))
adm1s_no_water$n_sample = adm1s_no_water$n_mines * 2
...
```{r}
comb_mining_sampling_pts = rbind(maus_sampling_pts%>%rename(geom = geometry), snp_filtered)
...
```{r}
adm1s_no_water = rmapshaper::ms_erase(non_mine_full%>%filter(gn_name != "Chukotskiy Avtonomnyy Okrug"), water_sf)%>%
  st_make_valid()
rm(adm1s_filtered)
...
```{r}
# Sample points
sampled_points <- map2_dfr(
  adm1s_no_water$geometry,
  adm1s_no_water$n_sample,
  ~ if (.y > 0) {
      pts <- st_sample(.x, size = .y, type = "regular")
      st_sf(geometry = pts) |> st_set_crs(crs)} 
  else {NULL})

# NUMBER OF SAMPLING POINTS DOESN'T SEEM TO LINE UP. TOO MANY IN SOME ADM1S, TOO FEW IN OTHERS

sampled_points%>%
  st_join(., adm1s_no_water%>%select(id, gn_name, geonunit))%>%
  data.frame()%>%
  arrange(id)%>%
  group_by(id, gn_name, geonunit)%>%
  reframe(n = n())

adm1s_no_water%>%
  data.frame()%>%
  select(id, 1:2, n_mines, n_sample)
