...
snp_in_maus = snp_study_area%>%
  st_join(., st_buffer(maus_filtered, 5000))
  st_filter(., maus, .predicate = st_within)%>%
...
tm_shape(st_buffer(maus_filtered%>%filter(geonunit == "Argentina"), 5000))+
  tm_polygons()+
...
The S&P dataset is already points so let's sample points from the Maus dataset and combine the two.
```{r}
set.seed(1236)

# Sample one point per polygon
sampled_pts <- lapply(st_geometry(maus), function(poly) {
  pt <- st_sample(poly, size = 1, type = "random", exact = TRUE)
  if (length(pt) == 0) st_point(c(NA_real_, NA_real_)) else pt[[1]]})

# Combine into a POINT geometry list
pts_sfc <- st_sfc(sampled_pts, crs = st_crs(maus))

# Create a new sf object (copying attributes if desired)
maus_sampling_pts <- st_sf(geometry = pts_sfc)%>%
  st_join(., maus)

tm_shape(maus%>%filter(gn_name == "Otago"))+
  tm_polygons()+
