...
test = all_sampling_pts %>%
  ...
  left_join(., adm1s_filtered%>%select(adm1_id, landmark_community)%>%st_drop_geometry(), by = "adm1_id")%>%
...
match_all = matchit(
  ...
  exact = ~ adm1_id)
match_all
save(match_all, file = "match_all.Rdata")
#load("match_all.Rdata")
...
match_recog = matchit(
  ...
  caliper = 0.2)
match_recog
save(match_recog, file = "match_recog.Rdata")
load("match_recog.Rdata")
...
match_unrecog = matchit(
  ...
  caliper = 0.1)
match_unrecog
save(match_unrecog, file = "match_unrecog.Rdata")
load("match_unrecog.Rdata")


match_community = matchit(
  stratum_mine ~ road_density + pop_density + dist_water + forest_cover + cropland_cover + protected,
  data = test%>%filter(landmark_community == 1 & (!is.na(treatment_unrecog) | !is.na(treatment_recog))),
  method = "nearest",
  exact = ~ adm1_id,
  caliper = 0.2)
save(match_community, file = "match_community.Rdata")
...
plot_balance <- function(match_data, threshold = 0.1) {
  ...
  love <- love.plot(match_data, threshold = threshold, abs = T)
...
model_base = glm(
  treatment_all ~ stratum_mine + road_density + pop_density +
...
# Run McNemar's test
mcnemar.test(table(match_all_df$stratum_mine, match_all_df$treatment_all))
chisq.test(match_all_df$stratum_mine, match_all_df$treatment_all)

summary(feglm(treatment_all ~ stratum_mine + road_density + pop_density +
...
# Run McNemar's test
mcnemar.test(table(match_unrecog_df$stratum_mine, match_unrecog_df$treatment_unrecog))
