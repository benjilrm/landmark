...
```{r, eval = F}
st_write(usgs_deposits, "Processed data/usgs_deposits_filtered.gpkg")
```
...
Export processed dataset
```{r, eval = F}
st_write(snp, "Processed data/snp_points_filtered.gpkg")
```
...
Export processed dataset
```{r, eval = F}
st_write(maus_joined, "Processed data/maus_polygons_filtered_joined.gpkg", delete.dsn = T)
```
