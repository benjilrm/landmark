...
owen_commodities = as.vector(ecm$`ETM list`)
```
...
```{r}
countries = st_transform(ne_countries(scale = "medium"), crs)

countries%>%data.frame()%>%filter(sovereignt == "France")
#%>%filter(grepl("Franc", subunit))
```
...
```{r}
usgs_deposits = st_read("Raw data/Global Distribution of Critical Minerals (USGS)/PP1802_CritMin_pts.shp")%>%
...
```{r}
snp = st_read("Raw data/S&P (full)/S&P/MiningAssetsSHP.shp")%>%
...
```{r}
maus = st_read("C:/Users/benji/Downloads/Land_Tenure_Paper-20250428T162244Z-001/Land_Tenure_Paper/Filtered_Mines_Maus_Jasanski.geojson")

maus = st_read("Raw data/Maus et al/Global_Mining_Polygons_v2.shp")
```
...
```{r}
within_distance <- st_is_within_distance(maus, snp, dist = maus_threshold)

maus_filtered <- maus[apply(within_distance, 1, any), ]%>%
...
maus_countries = data.frame(table(maus_joined$country))%>%
...
```{r}
mausc = unique(sort(maus_joined$country))
snpc = unique(sort(snp$country))
...
```{r}
write.csv(country_master, "Processed data/data_breakdown_by_country_apr29.csv", row.names = F)
```
